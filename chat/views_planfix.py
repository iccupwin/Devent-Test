...
def planfix_tasks(request):
    ...
    if should_use_apple_style(request):
        return planfix_tasks_apple(request)
...
def planfix_tasks_apple(request):
    """Отображение списка задач из Planfix с Apple дизайном"""
    try:
        # Базовые параметры запроса
        page = int(request.GET.get('page', 1)) - 1  # Страницы в UI с 1, в API с 0
        status_filter = request.GET.get('status', 'all')
        ...
        # Логируем информацию о запросе
        logger.info(f"Запрос задач Planfix (Apple-стиль): страница={page+1}, статус={status_filter}, всего={total_count}")
        
        return render(request, 'chat/planfix_task_apple.html', {
            'tasks': tasks,
            'total_count': total_count,
            'current_page': page + 1,
            'status_filter': status_filter,
        ...
        logger.error(f"Ошибка при получении задач Planfix (Apple-стиль): {str(e)}", exc_info=True)
        return render(request, 'chat/planfix_task_apple.html', {
            'tasks': [],
            'total_count': 0,
            'current_page': 1,
...
def planfix_task_detail(request, task_id):
    ...
    # Проверка, использовать ли Apple-стиль
    if should_use_apple_style(request):
        return planfix_task_detail_apple(request, task_id)
...
def planfix_task_detail_apple(request, task_id):
    """Отображение детальной информации о задаче в стиле Apple"""
    try:
        # Получаем задачу из Planfix по ID
        task = get_task_by_id(task_id)
        
        # Добавляем логирование для отладки
        logger.info(f"Получена задача с ID {task_id} (Apple-стиль): {task}")
        
        if not task:
            # Добавляем обработку случая, когда задача не найдена
            logger.error(f"Задача с ID {task_id} не найдена (Apple-стиль)")
            # Перенаправляем на страницу со списком задач
            return redirect('chat:planfix_tasks_apple')
        
        # Передаем задачу в шаблон
        return render(request, 'chat/planfix_task_detail_apple.html', {
            'task': task,
            'debug': False  # Можно установить в True для отладки
        })
        ...
        # Логируем ошибку
        logger.error(f"Ошибка при получении деталей задачи {task_id} (Apple-стиль): {str(e)}", exc_info=True)
        
        # Переходим на страницу со списком задач
        return redirect('chat:planfix_tasks_apple')
